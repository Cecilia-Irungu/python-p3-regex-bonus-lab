...
def findall(pattern="**", root="."):
    """
    findall(pattern="**", root=".") -> list of paths
    - If pattern=="**" it returns all files under root (recursive).
    - Uses glob with recursive=True, returns full paths.
    """
    if pattern == "**":
        pat = os.path.join(root, "**", "*")
    else:
        pat = os.path.join(root, pattern)
    return [p for p in glob(pat, recursive=True) if os.path.isfile(p)]
